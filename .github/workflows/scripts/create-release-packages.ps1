...
.PARAMETER Agents
    ...
    Valid agents: claude, gemini, copilot, VSCODE-agent, qwen, opencode, windsurf, codex, kilocode, auggie, roo, codebuddy, amp, q, bob, qoder
...
function Build-Variant {
    ...
    switch ($Agent) {
        ...
        'VSCODE-agent' {
            $cmdDir = Join-Path $baseDir ".VSCODE/commands"
            Generate-Commands -Agent 'VSCODE-agent' -Extension 'md' -ArgFormat '$ARGUMENTS' -OutputDir $cmdDir -ScriptVariant $Script
        }
        'qwen' {
...
$AllAgents = @('claude', 'gemini', 'copilot', 'VSCODE-agent', 'qwen', 'opencode', 'windsurf', 'codex', 'kilocode', 'auggie', 'roo', 'codebuddy', 'amp', 'q', 'bob', 'qoder')
$AllScripts = @('sh', 'ps')
